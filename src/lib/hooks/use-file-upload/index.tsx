...
export const useFileUpload: UseFileUploadHook = (options) => {
  ...
  const { startUpload, permittedFileInfo, isUploading } = useUploadThing(endpoint, {
    onClientUploadComplete: (res) => onUploadComplete(res.map((r) => omit(r, ['serverData']))),
    onUploadError: (err) => onUploadError(err.message),
    onUploadBegin,
  });

  if (!endpoint || typeof endpoint !== 'string') throw new Error('No endpoint provided for File Upload');
