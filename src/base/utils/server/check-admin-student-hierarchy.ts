...
export const checkAdminStudentHierarchy = async (studentId: number) => {
  ...
  const membership = memberships.find((m) => m.publicUserData?.userId === sesh.userId);
  ...
  const studentMembership = memberships.find((m) => m.publicUserData?.userId === studentDb.clerkUserId);
